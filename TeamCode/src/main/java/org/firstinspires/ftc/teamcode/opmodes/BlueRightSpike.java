...
public class BlueRightSpike extends OpMode {
    ...
    public void init() {
        ...
        TimeManager.reset();
        hMap.odometerWheels.setBeginningPose(DukConstants.AUTOMATED_CONTROLLER_PARAMS.AUTON_BEGIN_POSES.BLUE_RIGHT_X,
                DukConstants.AUTOMATED_CONTROLLER_PARAMS.AUTON_BEGIN_POSES.BLUE_RIGHT_Y,
                DukConstants.AUTOMATED_CONTROLLER_PARAMS.AUTON_BEGIN_POSES.BLUE_RIGHT_H,
                true);
        assessedDiagnostics = new AssessedDiagnostics(hMap);
        autonTaskExecuter = new AutonTaskExecuter();
        autonTaskExecuter.tasks.add(new AutonPointTask(-46900, 61800, (float)Math.toRadians(180), 0, 0));
